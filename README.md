...
点击「去屏幕上点选」，当前可见区域会显示带名称的虚线框，点框内即可选中；也可以直接点未标框的位置，再用「选外层」选容器。辅助框最多显示 32 个，优先标出底部和顶栏控件，避免满屏嵌套。只在点选期间采样可见区域，滚动、尺寸或内容变化时合并更新；退出后清除辅助框和监听，不扫描整个聊天记录。选中后显示对应的宽度、高度、文字颜色、背景颜色、透明度、位置、圆角和边框；行内文字不会提供无效的宽高控件。点击「选外层」可从图标或按钮切到它的容器，已点选列表可切回之前的元素。头像和底栏原有功能仍通过「快捷部位」访问。

通用点选默认针对选中的位置，不自动批量修改同类元素；修改父容器的颜色或透明度可能影响内部内容。优先以元素 ID 定位，无唯一 ID 时使用结构路径，聊天或界面结构改变后需重新确认。CSS 伪元素、iframe 内部及封闭的 Shadow DOM 暂不单独点选；点选只编辑外观，不编辑消息或执行原按钮动作。
...
- 已移除全部字号编辑与字体探测，不再临时改动酒馆字体比例或字号。输入框提示文字和颜色调整保留。
- 若检测到 v1.7 通用点选保存的字号覆盖，会显示「清理旧版字号修改」，仅清理同时带工作室区块和选择器标记的 `font-size`；可预览、撤销，保存后写入当前美化。早期无独立标记的正文字号规则不能可靠区分来源，保留原样，需人工核对。
...
通用点选可调整顶栏、输入框和气泡等可见元素，复杂非像素位移暂不支持；已通过自动测试与模拟宿主的桌面/手机视口验证，真实 ST/TT 与手机 WebView 仍待回归。详见 [可视化微调说明](docs/VISUAL-EDITOR.md)。
