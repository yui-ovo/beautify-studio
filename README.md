...
点击「去屏幕上点选」，再点可见的按钮、文字、图标、图片或区域。选中后显示对应的宽度、高度、字号、文字颜色、背景颜色、透明度、位置、圆角和边框；行内文字不会提供无效的宽高控件。点击「选外层」可从图标或按钮切到它的容器，已点选列表可切回之前的元素。头像和底栏原有功能仍通过「快捷部位」访问。

通用点选默认针对选中的位置，不自动批量修改同类元素；修改父容器的字号、颜色或透明度可能影响内部内容。优先以元素 ID 定位，无唯一 ID 时使用结构路径，聊天或界面结构改变后需重新确认。CSS 伪元素、iframe 内部及封闭的 Shadow DOM 暂不单独点选；点选只编辑外观，不编辑消息或执行原按钮动作。
