...
function openPanel(preferredDocument = null) {
  ...
  const renderBulkList = () => {
    ...
    for (const theme of themes) {
      ...
      checkbox.checked = root.__bulkSelected?.has(theme.name) || false;
      checkbox.addEventListener("change", () => {
        root.__bulkSelected ||= /* @__PURE__ */ new Set();
        checkbox.checked ? root.__bulkSelected.add(theme.name) : root.__bulkSelected.delete(theme.name);
        updateBulkCount();
      });
      const name = getHostDocument().createElement("span");
      name.textContent = theme.name;
  ...
  const updateBulkCount = () => {
    const count = root.__bulkSelected?.size || 0;
    bulkCount.textContent = `已选 ${count} 款`;
    root.querySelector(".bulk-confirm-delete").disabled = !count || busy;
  };
  root.querySelector(".batch-delete").addEventListener("click", () => {
    if (busy) return;
    root.__bulkSelected = /* @__PURE__ */ new Set();
    bulkSearch.value = "";
  ...
  root.querySelector(".bulk-select-all").addEventListener("click", () => {
    root.__bulkSelected ||= /* @__PURE__ */ new Set();
    ...
    visible.forEach((input2) => {
      input2.checked = !all;
      input2.checked ? root.__bulkSelected.add(input2.value) : root.__bulkSelected.delete(input2.value);
    });
    updateBulkCount();
  });
  root.querySelector(".bulk-confirm-delete").addEventListener("click", async () => {
    const names = [...root.__bulkSelected || []];
